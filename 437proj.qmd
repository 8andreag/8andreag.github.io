...
To understand my analysis, it’s also important to have some background knowledge about education systems, student performance metrics, and social factors that influence learning. In Portugal, and for this data in particular the target attribute G3 has a strong correlation with attributes G2 and G1. This occurs because G3 is the final year grade (issued at the 3rd trimester), while G1 and G2 correspond to the 1st and 2nd trimester grades. It is more difficult to predict G3 without G1 and G2. $\textbf{Grades are typically on a 0–20 scale, with 10 as the minimum passing mark.}$ $\textbf{Compared to here in the U.S. high school years are generally sectioned}$ $\textbf{into semesters and students receive letter grades A, B, C, D, or F.}$

Student performance refers to how well a student meets academic benchmarks, typically measured through grades. Performance can be influenced by various factors, including socioeconomic background, parental involvement, and individual learning styles.
...
* $\textbf{famsize:}$ family size (binary: 'LE3' - ≤ 3 or 'GT3' - > 3)
...
* $\textbf{Mjob:}$ mother's job (nominal: 'teacher', 'health' care related, civil 'services' (e.g. administrative or police), 'at_home' or 'other')
...
* $\textbf{traveltime:}$ home to school travel time (numeric: 1 - <15 min., 2 - 15 to 30 min., 3 - 30 min. to 1 hour, or 4 - >1 hour)

* $\textbf{studytime:}$ weekly study time (numeric: 1 - <2 hours, 2 - 2 to 5 hours, 3 - 5 to 10 hours, or 4 - >10 hours) failures: number of past class failures (numeric: n if 1<=n<3, else 4)
...
* $\textbf{nursery:}$ attended nursery school (binary: yes or no) higher: wants to take higher education (binary: yes or no)
...
$\textbf{We will be working with just a few varaibles:}$
...
$\textbf{The main response variables will be:}$
...
These are variables I think are important predictors for academic performance, as study time, parental education and family support can significantly impact a student’s success.
...
The first summary reveals that final grades tend to increas as the fathers level of education increases.
...
It is somewhat evident that study time does effect the students grades  
...
Again, as expected there are very high positive correlations between G1 G2 and G3.

This analysis confirms that a linear model using G1 and G2 to predict G3 is appropriate.

## Modeling

For this section I think that using PCA and clustering are good choices for this data, especially since my goal is exploration and inference rather than strict prediction. 

$\textbf{Why PCA and Clustering?}$

There are a lot of variables and we want to get the most out of our data without it being overly complicated and PCA will help with that. Clustering groups similar data points into clusters, revealing patterns and structures within the data.

PCA and clustering are both unsupervised learning techniques that help uncover hidden structure in the data.

PCA reduces dimensions by identifying the directions (principal components) that explain the most variance in the data. It helps reveal underlying patterns while simplifying the feature space.

Clustering groups similar observations together based on selected features. It helps explore whether certain student profiles naturally emerge and whether those profiles relate to academic performance or support structures.
...
student_pca_recipe <- recipe(G3 ~ ., data = student_train) |>
  # Remove response variable (G3) for later comparison
...
student_pca_baked <- bake(student_pca_prep, new_data = NULL) |> select(-G3)
...
#Runs Principal Component Analysis on preprocessed dataset using prcomp().
...
Although only a smaller portion of the total variance is retained, about 39%, these first two components still capture the most dominant axes of variation in the data. They are particularly useful for uncovering meaningful group structure (based on academic performance or support) and identifying how key variables relate to one another in reduced space. 

The trade-off is that this level of reduction sacrifices some detail but it significantly enhances interpretability and visual insight, which is appropriate given the objectives of this part of the analysis.
...
biplot(student_pca_model,
       scale = 0,
       cex = 0.9,            # Shrinks text size for better readability
...
random_rows <- sample(nrow(student_pca_model$x), 100)
...
First off, there are only two components: PC1 (horizontal axis) and PC2 (vertical axis). Second, the numbers (-1.0 to 1.0) represent standardized scores, recall thatnthey are scaled, so each axis is measured in standard deviation units. Third, each arrow on the plot represents a variable (feature) from the dataset and are positioned in this new 2D space based on how much it contributes to or aligns with these components. The direction shows how strongly that variable is aligned with each principal component. Arrows pointing to the right contribute more to PC1. Similaryly, arrows pointing up contribute more to PC2. Their lengths represent their importance. The longer the arrow, the more important that variable is in shaping the component. Shorter arrows mean the variable has less influence on PC1 and PC2. With this knowlege we can no interpret the biplot.
...
$\text{PC1} = 0.2611\times(Fedu) + 0.2861\times(Medu) + 0.5105\times(G1) + 0.5261\times(G2) + 0.5180(G3) - 0.0290\times(Pstatus = T) + 0.0052\times(guardian = mother) - 0.0968\times(guardian = other) + 0.0196\times(studytime = X2) + 0.1394\times(studytime = X3) + 0.0795\times(studytime = X4) + 0.0890\times(famsup = yes)$

$\text{PC2} = 0.4408\times(Fedu) + 0.4550\times(Medu) - 0.1584\times(G1) - 0.1785\times(G2) + 0.5180(G3) - 0.0290\times(Pstatus = T) + 0.0052\times(guardian = mother) - 0.0968\times(guardian = other) + 0.0196\times(studytime = X2) + 0.1394\times(studytime = X3) + 0.0795\times(studytime = X4) + 0.0890\times(famsup = yes)$
This equation shows how PC1 and are computed as a linear combinations of the original variables, each weighted by a coefficient (loading).

The larger the absolute value of a coefficient, the more that variable contributes to PC1.

The sign (positive/negative) tells you the direction of that influence.

4. Clustering: Apply K-means and Evaluate
```{r}
...
For Clustering:
Use total within-cluster sum of squares and silhouette score.
...
Silhouette score: Measures how well-separated the clusters are. Closer to 1 = good separation.

Summary (Narrative You Can Use)
I used PCA to reduce dimensionality and identify the most important features underlying student characteristics. After standardizing and transforming the data, I retained the first few principal components that explained the majority of the variance.

I then applied k-means clustering to the PCA-transformed data to identify natural groupings of students. I chose k = 3 to balance interpretability and compactness. Model performance was evaluated using total within-cluster sum of squares and silhouette scores, which suggested that the clusters were moderately well-defined. Further analysis showed how these clusters related to variables like study time, support, and academic performance.
